    ...
    public partial class DomesticRequest : System.Web.UI.Page
    {
        ...
        protected void submitRequestbtn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Session["filename"] != null && Session["userID"] != null)
                {
                    ...
                    using (var db = new SqlConnection(connectionString))
                    {
                        ...
                        using (var cmd = db.CreateCommand())
                        {
                            ...
                            cmd.CommandText = "INSERT INTO travelRequest (travelRequestID, travelHomeFacility, travelEmpID, travelFname, travelMname, travelLname , travelLevel, travelMobilenum, travelProjectCode, travelFrom, travelDeparture, travelReturn, travelPurpose, travelReqStatus, travelManager, travelRemarks, travelTo, travelOthers, travelType, travelOptions, travelUserID, travelProofname, travelProofPath, travelDateCreated)"
                                + "VALUES (@ID, @location, @empID, @empFName, @empMName, @empLName, @level, @mobile, @projCode, @from, @departure, @return, @purpose, @reqStatus, @manager, @remarks, @destination, @others, @type, @options, @userID, @proofname, @proofpath, @created)";
            ...
            try
            {
                using (var db = new SqlConnection(connectionString))
                {
                    ...
                    using (var cmd = db.CreateCommand())
                    {
                        cmd.Parameters.Clear();
                        cmd.CommandType = CommandType.Text;
                        cmd.CommandText = "UPDATE travelRequest SET travelDateSubmitted = @dateSubmitted WHERE travelRequestID = @ID";
                        ...
                        if (rowsAffected > 0)
                        {
                           ...
                           Response.Write("<script>alert ('Domestic Travel Request Submitted!'); window.location.href = 'EmployeeDashboard.aspx'; </script>");
                        }
                        else
                        {
