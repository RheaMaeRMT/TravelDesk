...
namespace TravelDesk.Employee
{
    public partial class InternationalRequest : System.Web.UI.Page
    {
        ...
        protected void btnUpload_Click(object sender, EventArgs e) //UPLOAD APPROVAL PDF
        {
            try
            {
        ...
        private void displayContents()
        {
            ...
            string otherSelected = employeePurpose.SelectedItem.Value;
                ...
                using (var db = new SqlConnection(connectionString))
                {
                    ...
                    using (var cmd = db.CreateCommand())
                    {
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            Response.Write("<script>alert ('International Travel Request Submitted!'); window.location.href = 'EmployeeDashboard.aspx'; </script>");
        ...
        private void insertDraftRoute(string requestID)
        {
            Random ranID = new Random();
            int random = ranID.Next(100000, 999999);
        ...
        protected void uploadPassportbtn_Click(object sender, EventArgs e) //UPLOAD PASSPORT PDF
        {
            try
            {
                if (Session["userName"] != null)
                    ...
                    if (passportUpload.HasFile)
                    {
                        ...
                        if (extension == ".pdf") // Allow only PDF files
                        {
                            if (filesize < 4100000)
                            {
                                ...
                                // Show the PDF viewer for passport 
                                ...
                                pdfViewer.Attributes["src"] = approvalPath;
                                pdfBlock.Style["display"] = "block";
